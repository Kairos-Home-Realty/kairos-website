...
export default function AuroSapphirePage() {
  ...
  return (
    <>
      ...
      {project.galleryImages && <section id="plans" className="section-pad scroll-mt-24 bg-offwhite"><div className="container-xl px-6 lg:px-12"><FadeIn><div className="mb-8 max-w-2xl"><span className="text-xs font-semibold uppercase tracking-[0.25em] text-gold-dark">Supplied Auro Realty brochure</span><h2 className="mt-3 font-display text-3xl font-semibold text-navy md:text-4xl">Location, plans and amenities</h2><p className="mt-3 text-sm leading-relaxed text-slate/70">Select an image to open the brochure page at a larger size.</p></div></FadeIn><div className="grid gap-5 md:grid-cols-2">{project.galleryImages.map((image) => <a key={image.src} href={image.src} target="_blank" rel="noopener noreferrer" className="block overflow-hidden rounded-xl2 border border-navy/10 bg-white p-2 shadow-card"><Image src={image.src} alt={image.alt} width={1920} height={1080} className="h-auto w-full rounded-lg" unoptimized /><p className="px-3 py-3 text-sm text-slate/70">{image.alt} · Open larger image</p></a>)}</div></div></section>}
